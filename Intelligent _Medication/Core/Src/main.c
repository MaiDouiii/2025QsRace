...
int main(void)
{
  ...
  IIC_init();
	...
	for (int i = 0; i < 3; i++)
	{
			user_messages[i].send_flag[0] = 0;
			user_messages[i].send_flag[1] = 0;
			user_messages[i].send_flag[2] = 0;
			for (int j = 0; j < 3; j++)
			{
					user_messages[i].Time[j][0] = 0;
					user_messages[i].Time[j][1] = 0;
					for (int k = 0; k < 6; k++)
					{
							user_messages[i].medicine[j][k] = 0;
					}
			}
	}
